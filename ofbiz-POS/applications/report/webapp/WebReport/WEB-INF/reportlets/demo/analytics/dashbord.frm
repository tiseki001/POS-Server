...
where year(now())-year(birthdate)<30
union
SELECT '30岁到40岁' as age,empid FROM EMPLOYEE
where year(now())-year(birthdate)>=30
and year(now())-year(birthdate)<40
union 
SELECT '大于40岁' as age,empid FROM EMPLOYEE
where year(now())-year(birthdate)>=40) a","EMPLOYEE")} GROUP BY ${type}
...
<Attr isNullValueBreak="true" autoRefreshPerSecond="-1" seriesDragEnable="false" plotStyle="5"/>
<newHotTooltipStyle>
<AttrContents>
