...
//              Debug.logInfo("=====product found:" + productId + " and features: " + selectedFeatures);
    ...
    public static Boolean setAllProdcutFeature(Delegator delegator, String productId){
    	List<GenericValue> featureList = null;
    	List<EntityCondition> condList = FastList.newInstance();
    	...
    	String allFeature = "";
    	try {
			featureList = delegator.findList("ProductFeatureAndType", condition, null, null, null, true);
			for (GenericValue feature: featureList) {
				allFeature += feature.get("typeDescription").toString()+":"+feature.get("featurDescription").toString()+";";
			}
			allFeature = allFeature.substring(0, allFeature.length()-1);
			Map<String, Object> pkMap = FastMap.newInstance();
			pkMap.put("productId", productId);
			GenericValue entity;

			entity = delegator.findByPrimaryKey("Product", pkMap);
			if (UtilValidate.isNotEmpty(entity)) {
				entity.set("featureAll", allFeature);
				entity.store();
			}
    	...
    	//featureList.get(0).keySet()
    	return true;
    }
    
    
    public static Boolean setAllProdcutCategory(Delegator delegator, String productId){
    	List<GenericValue> featureList = null;
    	List<EntityCondition> condList = FastList.newInstance();
    	condList.add(EntityCondition.makeCondition("productId", productId));
    	...
    	String allFeature = "";
    	try {
			featureList = delegator.findList("ProductCategoryAndMember", condition, null, null, null, true);
			for (GenericValue feature: featureList) {
				allFeature += feature.get("description").toString()+";";
			}
			allFeature = allFeature.substring(0, allFeature.length()-1);
			Map<String, Object> pkMap = FastMap.newInstance();
			pkMap.put("productId", productId);
			...
			entity = delegator.findByPrimaryKey("Product", pkMap);
			if (UtilValidate.isNotEmpty(entity)) {
				entity.set("categoryAll", allFeature);
				entity.store();
			}
    ...
    public static Boolean initInventorynItem(Delegator delegator, String productId){
    	List<GenericValue> storeList = null;
    	List<GenericValue> facilityList = null;
    	List<EntityCondition> condList = FastList.newInstance();
    	String storeId;
    	GenericValue inventorynItem;
    	try {
    		storeList = delegator.findByAnd("PosStoreInfo",null,null);
    		if (!UtilValidate.isNotEmpty(storeList)||storeList.size()<1) {
    			facilityList = delegator.findByAnd("FacilityAndStore",null,null);
    			
    		}else{
    			storeId = storeList.get(0).getString("storeId");
    			facilityList = delegator.findByAnd("FacilityAndStore",UtilMisc.toMap("productStoreId",storeId),null);
    		}
    		for (GenericValue facility: facilityList) {
    			if(UtilValidate.isEmpty(
       					delegator.findByPrimaryKey("InventorynItem", UtilMisc.toMap("inventoryItemId",facility.get("facilityId")+productId)))){
       			
    			inventorynItem = GenericValue.create(delegator.getModelEntity("InventorynItem"));
    			inventorynItem.set("inventoryItemId", facility.get("facilityId")+productId);
       			inventorynItem.set("inventoryItemType", "SERIALIZED_INV_ITEM");
       			inventorynItem.set("onhand", 0L);
       			inventorynItem.set("promise", 0L);
       			inventorynItem.set("unitCost", "");
       			inventorynItem.set("productStoreId", facility.get("productStoreId"));
       			inventorynItem.set("productId", productId);
       			inventorynItem.set("partyId", facility.get("ownerPartyId"));
       			inventorynItem.set("facilityId", facility.get("facilityId"));
       					inventorynItem = delegator.makeValue("InventorynItem",inventorynItem);
       				inventorynItem.create();
       			}
    ...
    public static Boolean initInventorynItemByFacility(Delegator delegator, String facilityId,String productStoreId){
    	List<GenericValue> storeList = null;
    	...
    	try {
    		...
    		storeList = delegator.findByAnd("PosStoreInfo",null,null);
    		if (!UtilValidate.isNotEmpty(storeList)||storeList.size()<1) {
    			productList = delegator.findByAnd("Product",null,null);
    			
    		}else{
    			storeId = storeList.get(0).getString("storeId");
    			if(storeId.equals(productStoreId))
    				productList = delegator.findByAnd("Product",null,null);
    			else
    				return true;
    		}
    		for (GenericValue product: productList) {
    			inventorynItem = GenericValue.create(delegator.getModelEntity("InventorynItem"));
    			inventorynItem.set("inventoryItemId", facilityId+product.get("productId"));
       			inventorynItem.set("inventoryItemType", "SERIALIZED_INV_ITEM");
       			inventorynItem.set("onhand", 0L);
       			inventorynItem.set("promise", 0L);
       			inventorynItem.set("unitCost", "");
       			inventorynItem.set("productStoreId", productStoreId);
       			inventorynItem.set("productId", product.get("productId"));
       			inventorynItem.set("partyId", facility.get("ownerPartyId"));
       			inventorynItem.set("facilityId", facilityId);
       			if(UtilValidate.isEmpty(
       					delegator.findByPrimaryKey("InventorynItem", UtilMisc.toMap("inventoryItemId",inventorynItem.get("inventoryItemId"))))){
       				inventorynItem = delegator.makeValue("InventorynItem",inventorynItem);
       				inventorynItem.create();
       			}
    ...
    public static Boolean updateOrCreateProductPrice(Delegator delegator, GenericValue productPrice){
    	EntityCondition mainCond = EntityCondition.makeCondition("baseTypeId", 
    			EntityOperator.EQUALS, productPrice.get("productPriceTypeId"));;
		...
		try {
			gvs = delegator.findList("ProductPriceRuleAndTypeView", mainCond, null, null, null, false);
			for (GenericValue gv : gvs) {
				String price = SalesPriceWorker.round(gv.get("percent").toString(), 
						gv.get("digit").toString(), 
						gv.get("diff").toString(), 
						productPrice.get("price").toString());
				pkMap.put("productPriceTypeId", (String)gv.get("productPriceTypeId"));
				GenericValue entity = delegator.findByPrimaryKey("ProductPrice", pkMap);
				if (UtilValidate.isNotEmpty(entity)) {
					entity.set("price", new BigDecimal(price));
					entity.store();
				}else {
			...
			/*gvs = delegator.findList("ProductPriceRuleAndTypeView", null, null, null, null, false);
			for (GenericValue gv : gvs) {
				if (gv.get("baseTypeId").equals(productPrice.get("productPriceTypeId"))) {
					String price = SalesPriceWorker.round(gv.get("percent").toString(), 
							gv.get("digit").toString(), 
							gv.get("diff").toString(), 
							productPrice.get("price").toString());
    ...
    public static Boolean createProductPrice(Delegator delegator, GenericValue product){
    	try {
			List<GenericValue> gvs = delegator.findByAnd("ProductPrice", 
					UtilMisc.toMap("productId", product.get("productId")));
			
			if (UtilValidate.isEmpty(gvs)) {
				delegator.makeValue("ProductPrice", UtilMisc.toMap("productId", product.get("productId"),
						"productPriceTypeId", "BASIC_PRICE",
    ...
    public static Boolean updateProductPriceList(DispatchContext dctx, String id){
    	...
    	try {
    		...
    		for (GenericValue priceRule : priceRules) {
    			products = findProductById(dctx, id, (String) priceRule.get("productPriceRuleId"));
    			for (GenericValue product : products) {
					if (product.get("productId").equals(id)) {
						salesPolicyRules = delegator.findByAnd("ProductSalesPolicyRule", 
								UtilMisc.toMap("productPriceRuleId", priceRule.get("productPriceRuleId")));
						for (GenericValue salesPolicyRule : salesPolicyRules) {
							createProductPriceList(delegator, product, 
									priceRule.get("productPriceRuleId").toString(), 
									salesPolicyRule.get("productSalesPolicyId").toString());
						}
					}
				}
			}
    ...
    public static List<GenericValue> findProductById(DispatchContext dctx, 
    		String productId, String productPriceRuleId) {
    	Delegator delegator = dctx.getDelegator();
        ...
        EntityCondition pCond = EntityCondition.makeCondition("productId", 
    ...
    public static void createProductPriceList(Delegator delegator, GenericValue product, 
		...
		if (b) {
			price = SalesPriceWorker.getProductPriceById(delegator, priceRuleId, (String)product.get("productId"));
    ...
    public static void setProductPriceTypeRule(Delegator delegator, String productPriceTypeId){
    	EntityCondition mainCond = EntityCondition.makeCondition(
    			...
    			EntityOperator.OR, 
				EntityCondition.makeCondition("parentTypeId", EntityOperator.EQUALS, "EMPLOYEE"));
    	
    	List<GenericValue> lists = null;
    	List<GenericValue> roleTypes = null;
    	try {
    		roleTypes = delegator.findList("RoleType", mainCond, null, null, null, false);
    		for (GenericValue roleType : roleTypes) {
    			lists = delegator.findByAnd("ProductPriceTypeRule", 
    					UtilMisc.toMap("productPriceTypeId",productPriceTypeId,
    							"roleTypeId", roleType.get("roleTypeId")));
    			if (UtilValidate.isEmpty(lists)) {
    				delegator.makeValue("ProductPriceTypeRule", 
    						UtilMisc.toMap("productPriceTypeId", productPriceTypeId,
    								"roleTypeId", roleType.get("roleTypeId"),
    								"isVisble","N")).create();
    			}
