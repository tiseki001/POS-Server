...
public class PriceServices {
    ...
    public static Map<String, Object> calculateProductPrice(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (UtilValidate.isNotEmpty(agreementId)) {
            try {
                List<GenericValue> agreementPrices = delegator.findByAnd("AgreementItemAndProductAppl", UtilMisc.toMap("agreementId", agreementId, "productId", productId, "currencyUomId", currencyDefaultUomId));
     ...
     * @return
     */
    public static Map<String, Object> deletePriceType(DispatchContext dctx, Map<String, ? extends Object> context) {
        Delegator delegator = dctx.getDelegator(); 
        String productPriceTypeId = (String)context.get("productPriceTypeId");
        String isBase = (String)context.get("isBase");
        ...
        try {
        	...
        	ProductPriceLists = delegator.findByAnd("ProductPriceList", UtilMisc.toMap("productPriceTypeId", productPriceTypeId));
        	if (isBase.equals("N")) {
        		ProductPriceRules = delegator.findByAnd("ProductPriceRule", UtilMisc.toMap("productPriceTypeId", productPriceTypeId));
			}else{
				ProductPriceRules = delegator.findByAnd("ProductPriceRule", UtilMisc.toMap("baseTypeId", productPriceTypeId));
			}
        	
        	delegator.removeAll(ProductPriceLists);
        	delegator.removeAll(ProductPriceRules);
        	productPriceType.remove();
		} catch (GenericEntityException e) {
