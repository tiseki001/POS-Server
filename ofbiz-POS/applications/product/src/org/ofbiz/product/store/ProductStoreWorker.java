...
public class ProductStoreWorker {
    ...
    public static List<GenericValue> getEmployee(Delegator delegator, String partyId, String productStoreId) {
       
        List<GenericValue> personAndStore = null;
        
        List<EntityCondition> condList = FastList.newInstance();
        
        condList.add(EntityCondition.makeCondition("roleTypeId", "EMPLOYEE"));
        if(UtilValidate.isNotEmpty(partyId))
        	condList.add(EntityCondition.makeCondition("ownerPartyId", partyId));
        	
        if(UtilValidate.isNotEmpty(productStoreId))
        	condList.add(EntityCondition.makeCondition("productStoreId", productStoreId));
        personAndStore = getPersonAndStore(delegator,condList);
        return personAndStore;
    }
    public static List<GenericValue> getCustomer(Delegator delegator, String partyId, String productStoreId) {
    ...
    public static List<GenericValue> getPersonAndStore(Delegator delegator, List<EntityCondition> condList) {
    	List<GenericValue> personAndStore = null;
        ...
        try {
        	personAndStore = delegator.findList("", condition, null, UtilMisc.toList("partyId"), null, true);
        } catch (GenericEntityException e) {
            Debug.logError(e, "Unable to get ProductStore shipping methods", module);
            return null;
        }
    ...
    public static Boolean setAllProdcutStoreAttribute(Delegator delegator, String productStoreId){
    	...
    	String allFeature = "";
    	try {
			featureList = delegator.findList("ProductStoreAttributeAndType", condition, null, null, null, true);
			for (GenericValue feature: featureList) {
				allFeature += feature.get("description").toString()+":"+feature.get("attrValue").toString()+";";
			}
			allFeature = allFeature.substring(0, allFeature.length()-1);
			Map<String, Object> pkMap = FastMap.newInstance();
			pkMap.put("productStoreId", productStoreId);
			GenericValue entity;

			entity = delegator.findByPrimaryKey("ProductStore", pkMap);
			if (UtilValidate.isNotEmpty(entity)) {
				entity.set("allAttribute", allFeature);
				entity.store();
			}
    ...
    public static BasePosObject getProductStores(DispatchContext dctx) {
		Delegator delegator = dctx.getDelegator();
		BasePosObject pObject = new BasePosObject();
		List<GenericValue> list = null;
		try {
			list = delegator.findList("ProductStoreRangeView", null, null, null, null, false);
			if (!UtilValidate.isEmpty(list)||list.size()<=0) {
				list = delegator.findList("ProductStore", null, null, null, null, false);
			}
