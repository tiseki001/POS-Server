...
<div class="screenlet">
    ...
    <form method='post' action='<@ofbizUrl>CreateProductPromoProductCond</@ofbizUrl>' id="_form_CreateProductPromoProduct" name="selectAllForm">
       <table border="1" class="basic-table" id="tab">
          ...
          <#assign rowCount = 2>
          <#list productPromoProductConds as productPromoProductCond>
          	<tr class="header-row" id="${rowCount}">
          	  <td><input type='hidden' name="rowCount_o_${rowCount}" id="rowCount${rowCount}" value="${rowCount}"/></td>
	          <td><input type='text' name="left1_o_${rowCount}" id="left1${rowCount}" value="${productPromoProductCond.left1?if_exists}" size='15'/></td>
              <td><input type='text' name="fieldName_o_${rowCount}" id="fieldName${rowCount}" value="${productPromoProductCond.fieldName?if_exists}" size='15'/></td>
              <td><input type='text' name="operate_o_${rowCount}" id="operate${rowCount}" value="${productPromoProductCond.operate?if_exists}" size='15'/></td>
              <td><input type='text' name="value1_o_${rowCount}" id="value1${rowCount}" value="${productPromoProductCond.value1?if_exists}" size='15'/></td>
              <td><input type='text' name="right1_o_${rowCount}" id="right1${rowCount}" value="${productPromoProductCond.right1?if_exists}" size='15'/></td>
              <td><input type='text' name="relation_o_${rowCount}" id="relation${rowCount}" value="${productPromoProductCond.relation?if_exists}" size='15'/></td>
	...
	if (true) {
      $("#_fieldName").change(function(e, data) {
		  ...
		  if(productText == "productId" || productText == "productName" || productText == "brandName"){
		  	$("#del").empty();	  //删除子元素
		  	$("#del").append("<input type='text' id='relation' name='relation' size='15'/>");
		  }else{
		  	$("#del").empty();	  //删除子元素
