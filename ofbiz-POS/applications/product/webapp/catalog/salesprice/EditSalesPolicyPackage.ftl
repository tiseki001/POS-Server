       ...
       <table border="1" class="basic-table" id="tab">
          <tr >
          	<td colspan="5" align="center">
          		<input type="hidden" name="productSalesPolicyId" value="${productSalesPolicyId}" />
          		<#if productSalesPolicyId == "000000">
	              <input type="hidden" name="productPriceRuleId" value="${productPriceRule.productPriceRuleId}" />
	            <#else>
          		  <input type="hidden" name="productPriceRuleId" value="${productPriceRuleId}" />
	            </#if>
          		<input type="hidden" name="expreType" value="${expreType}" />
          		<input type="hidden" name="rowTotal" id="rowTotal" value="0"/>
            </td>
           </tr>
          <tr class="header-row">
            <td ><b></b></td>
          ...
          <tr class="header-row">
            ...
            <td ><b>${uiLabelMap.Value1}</b>
            	<span class="ui-widget" id="del">
				    <select id="AddSalesPolicyPackage_value1" size="1" name="value1">
				        <option value=""></option>
				    </select>
				</span>
            </td>
...
<script>
	...
	if (true) {
      $("#AddSalesPolicyPackage_fieldName").change(function(e, data) {
	      var productText = $("#AddSalesPolicyPackage_fieldName option:selected").val();
		  if(productText == "productId" || productText == "productName" || productText == "brandName"){
		  	...
		  	$("#del").empty();
		  	$("#del").append(" <select id='AddSalesPolicyPackage_value1' size='1' name='value1'></select>");
		  }
          getDependentDropdownValues('getProductFeatureCategoryList', 'fieldName', 'AddSalesPolicyPackage_fieldName', 'AddSalesPolicyPackage_value1', 'productFeatureCategoryList', 'productFeatureId', 'description');
      });
      getDependentDropdownValues('getProductFeatureCategoryList', 'fieldName', 'AddSalesPolicyPackage_fieldName', 'AddSalesPolicyPackage_value1', 'productFeatureCategoryList', 'productFeatureId', 'description', '_none_');
    };
