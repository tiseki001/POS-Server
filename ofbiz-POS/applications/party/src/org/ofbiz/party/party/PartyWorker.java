...
public class PartyWorker {
    ...
    public static BasePosObject findMenuCByUserLoginId(Delegator delegator, String userLoginId) throws GenericEntityException {
        ...
        dynamicView.addMemberEntity("MR", "MenuRole");
        dynamicView.addMemberEntity("MPC", "MenuPosC");
        ...
        dynamicView.addAlias("PSR", "thruDate");
        dynamicView.addAlias("PSR", "partyId");
        dynamicView.addAlias("PSR", "productStoreId");
        dynamicView.addAlias("MPC", "meunId");
        dynamicView.addAlias("MPC", "meunName");
        dynamicView.addAlias("MPC", "parentMeunId");
        dynamicView.addAlias("MPC", "meunlevel");
        dynamicView.addAlias("MPC", "meunsort");
        dynamicView.addAlias("MPC", "meunPic");
        dynamicView.addAlias("MPC", "meunShortcuts");
        dynamicView.addAlias("MPC", "meunDll");
        dynamicView.addAlias("MPC", "meunClass");
        dynamicView.addAlias("MPC", "meunMethod");
        dynamicView.addAlias("MR", "isVisble");
        
        orderBy.add("meunsort");
        
        dynamicView.addViewLink("UL", "PER", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));
        dynamicView.addViewLink("UL", "PSR", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));
        dynamicView.addViewLink("PSR", "PS", Boolean.FALSE, ModelKeyMap.makeKeyMapList("productStoreId"));
        dynamicView.addViewLink("PSR", "MR", Boolean.FALSE, ModelKeyMap.makeKeyMapList("roleTypeId"));
        dynamicView.addViewLink("MR", "MPC", Boolean.FALSE, ModelKeyMap.makeKeyMapList("meunId"));
        
        fieldsToSelect.add("meunId");
        fieldsToSelect.add("meunName");
        fieldsToSelect.add("parentMeunId");
        fieldsToSelect.add("meunlevel");
        fieldsToSelect.add("meunsort");
        fieldsToSelect.add("meunPic");
        fieldsToSelect.add("meunShortcuts");
        fieldsToSelect.add("meunDll");
        fieldsToSelect.add("meunClass");
        fieldsToSelect.add("meunMethod");
        
        if (UtilValidate.isNotEmpty(userLoginId)) {
        	andExprs.add(EntityCondition.makeCondition("userLoginId", EntityOperator.EQUALS, userLoginId));
		}
        andExprs.add(EntityCondition.makeCondition("isVisble", EntityOperator.EQUALS, "Y"));
        andExprs.add(EntityCondition.makeCondition(EntityCondition.makeCondition("thruDate", EntityOperator.EQUALS, null), 
        		EntityOperator.OR, 
        ...
        BasePosObject posObj = new BasePosObject();
        // using list iterator
        EntityListIterator pli;
        List<GenericValue> lists = null;
        //List<GenericValue> listMap = null;
        
        Map<String, Object> map = FastMap.newInstance();
       /* List<GenericValue> userDetail = findUserDetail(delegator, userLoginId);
        if (UtilValidate.isNotEmpty(userDetail)) {
		...
		try {
			...
			lists = pli.getCompleteList();
			// map.put("menuDetail", lists);
			posObj.setFlag("S");
			posObj.setData(lists);
			pli.close();
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			posObj.setFlag("F");
			posObj.setMsg(e.getMessage());
			e.printStackTrace();
		}
    	return posObj;
    }
    
    public static GenericValue findUserDetail(Delegator delegator, String userLoginId, String storeId) throws GenericEntityException {
        ...
        dynamicView.addMemberEntity("PSR", "ProductStoreRole");
        dynamicView.addAlias("UL", "userLoginId");
        dynamicView.addAlias("PER", "partyId");
        dynamicView.addAlias("PER", "firstName");
        dynamicView.addAlias("PER", "middleName");
        dynamicView.addAlias("PER", "lastName");
        dynamicView.addAlias("PS", "storeName");
		...
		if (UtilValidate.isNotEmpty(storeId)) {
        	andExprs.add(EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, storeId));
		}
		...
		try {
        	...
        	//dynamicView.
        	System.out.println(mainCond.toString());
			pli = delegator.findListIteratorByCondition(dynamicView, mainCond, null, fieldsToSelect, orderBy, findOpts);
			lists = pli.getCompleteList();
    ...
    public static BasePosObject checkUserLogin(DispatchContext dctx, Locale locale, String userLoginId, String userLoginPwd, String storeID) throws GenericEntityException {
		...
		GenericValue visit = delegator.makeValue("Visit", UtilMisc.toMap("visitId", visitId));
		visit.create();
		
		Map<String, ? extends Object> context = UtilMisc.toMap("login.username", userLoginId, "login.password", userLoginPwd, "visitId", visitId, "locale", locale);
	    try {
	        // get the visit id to pass to the userLogin for history
	        // result = dispatcher.runSync("userLogin", UtilMisc.toMap("login.username", userLoginId, "login.password", userLoginPwd, "visitId", visitId, "locale", locale));
	        result =  LoginServices.userLogin(dctx, context);
	    ...
	    if ("success".equals(responseMessage)) {
	    	posObj.setFlag("S");
	    	posObj.setData(findUserDetail(delegator, userLoginId, storeID));
	 		posObj.setMsg(UtilProperties.getMessage(resourceError, 
	                "checkUserLoginSuccess", new Object[] {result.get("successMessage")}, locale));
		}else if("error".equals(responseMessage)){
			posObj.setFlag("F");
			if (result.get("errorMessage").equals("password error")) {
				posObj.setMsg(UtilProperties.getMessage(resourceError, 
						"checkUserLoginError", new Object[] { result.get("errorMessage")}, locale));
			}else if (result.get("errorMessage").equals("User not found.")){
				posObj.setMsg(UtilProperties.getMessage(resourceError, 
						"UserNotFound", new Object[] { result.get("errorMessage")}, locale));
			}else {
				posObj.setMsg(UtilProperties.getMessage(resourceError, 
						"LockUserLoginError", new Object[] { result.get("errorMessage")}, locale));
			}
		}
     	return posObj;
    }
