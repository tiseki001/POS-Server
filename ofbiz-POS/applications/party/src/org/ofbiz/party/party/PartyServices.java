...
import org.ofbiz.party.conUtil.condSqlString;
...
public class PartyServices {
    ...
    public static Map<String, Object> createPerson(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        if (party != null) {
            ...
            statusRec.set("isSync", Constant.ONE);
        ...
        person.set("isSync", Constant.ONE);
    ...
    public static Map<String, Object> createRoleType(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        List<GenericValue> menuPosCs = null;
        
        try {
        	menuPosCs = delegator.findList("MenuPosC", null, null, null, null, true);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		GenericValue menuRole = null;
		
		for (GenericValue menuPosC : menuPosCs) {
			menuRole = delegator.makeValue("MenuRole", UtilMisc.toMap("meunId", menuPosC.get("meunId"), 
					"roleTypeId", roleType.get("roleTypeId"),
					"menuType", "MenuPosC",
					"isVisble", "N"));
			try {
				menuRole.create();
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if ("Y".equals(lookupFlag)) {
            ...
            if (!"Y".equals(showAll)) {
                ...
                // modify the dynamic view
                if (UtilValidate.isNotEmpty(firstName) || UtilValidate.isNotEmpty(lastName) || UtilValidate.isNotEmpty(phoneMobile)) {
             ...
             // filter on lastName
                if (UtilValidate.isNotEmpty(phoneMobile)) {
                    paramList = paramList + "&phoneMobile=" + phoneMobile;
                    andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("phoneMobile"), EntityOperator.LIKE, EntityFunction.UPPER("%"+phoneMobile+"%")));
                }
    ...
    public static Map<String,Object> deleteRoleType(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
        String roleTypeId = (String)context.get("roleTypeId");
        GenericValue roleType = null;
        List<GenericValue> parentRoleTypes = null;
        
        try {
        	roleType = delegator.findByPrimaryKey("RoleType", UtilMisc.toMap("roleTypeId", roleTypeId));
        	parentRoleTypes = delegator.findByAnd("RoleType", UtilMisc.toMap("parentTypeId", roleTypeId));
        	delegator.removeAll(parentRoleTypes);
        	roleType.remove();
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		List<GenericValue> menuRoles = null;
		
		try {
			menuRoles = delegator.findByAnd("MenuRole", UtilMisc.toMap("roleTypeId", roleTypeId));
			delegator.removeAll(menuRoles);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    ...
    public static Map<String, Object> createMenuPosC(DispatchContext dctx, Map<String, ? extends Object> context) {
        Map<String, Object> result = FastMap.newInstance();
        Delegator delegator = dctx.getDelegator();
        Locale locale = (Locale) context.get("locale");
        GenericValue menuPosC = null;
        String meunId = (String)context.get("meunId");

        if (UtilValidate.isEmpty(meunId)) {
        	meunId = delegator.getNextSeqId("MenuPosC");
            try {
            	menuPosC = delegator.makeValue("MenuPosC", UtilMisc.toMap("meunId", meunId));
            	menuPosC.setNonPKFields(context);
            	menuPosC = delegator.create(menuPosC);
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
                return ServiceUtil.returnError(UtilProperties.getMessage(resource,
                        "PartyCannotCreateRoleTypeEntity",
            ...
            List<GenericValue> roleTypes = null;
            
            try {
            	roleTypes = delegator.findList("RoleType", null, null, null, null, true);
    		} catch (GenericEntityException e) {
    			// TODO Auto-generated catch block
    			e.printStackTrace();
    		}
    		...
    		for (GenericValue roleType : roleTypes) {
    			menuRole = delegator.makeValue("MenuRole", UtilMisc.toMap("meunId", meunId, 
    					"roleTypeId", roleType.get("roleTypeId"),
    					"menuType", "MenuPosC",
    					"isVisble", "N"));
    			try {
        ...
        if (menuPosC != null) {
            result.put("menuPosC", menuPosC);
        }
        return result;
    }
    public static Map<String,Object> deleteMenuPosC(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
        String meunId = (String)context.get("meunId");
        GenericValue menuPosC = null;
        List<GenericValue> parentMenuPosCs = null;
        
        try {
        	menuPosC = delegator.findByPrimaryKey("MenuPosC", UtilMisc.toMap("meunId", meunId));
        	parentMenuPosCs = delegator.findByAnd("MenuPosC", UtilMisc.toMap("parentMeunId", meunId));
        	delegator.removeAll(parentMenuPosCs);
        	menuPosC.remove();
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			menuRoles = delegator.findByAnd("MenuRole", UtilMisc.toMap("meunId", meunId, "menuType", "MenuPosC"));
			delegator.removeAll(menuRoles);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
     ...
     * @return
     */
    public static Map<String,Object> updateMenuRole(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
        String meunId = (String)context.get("meunId");
        String roleTypeId = (String)context.get("roleTypeId");
        String menuType = (String)context.get("menuType");
        String _rowSubmit = (String)context.get("_rowSubmit");
        
        if (UtilValidate.isEmail(_rowSubmit)) {
        	_rowSubmit = "N";
		}
     ...
     * @return
     */
    public static Map<String,Object> getMenuName(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
        String meunId = (String)context.get("meunId");
        String menuType = (String)context.get("menuType");
        
        Map<String, String> menu = UtilMisc.toMap("meunId", meunId);
        GenericValue gv = null;
        
        if (menuType.equals("MenuPosC")) {
			try {
				gv = delegator.findByPrimaryKey("MenuPosC", menu);
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}else if(menuType.equals("MenuPosS")) {
			try {
				gv = delegator.findByPrimaryKey("MenuPosS", menu);
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}else if(menuType.equals("MenuPosM")) {
			try {
				gv = delegator.findByPrimaryKey("MenuPosM", menu);
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        ...
        Map<String, Object> result = ServiceUtil.returnSuccess();
        result.put("menuName", gv.get("meunName"));
		return result;
	}
    /**
     * 根据登陆id取得对应用户 权限菜单
     ...
     * @return
     */
    public static Map<String, Object> getMenuCByUserLoginId(DispatchContext dctx, Map<String, ? extends Object> context) {
		Delegator delegator = dctx.getDelegator();
        ...
        if (UtilValidate.isNotEmpty(jsonStr)) {
        	...
        	userLoginId = (String) jsonObj.getString("userLoginId");
        }
        
        BasePosObject posObj = null;
        try {
        	posObj = PartyWorker.findMenuCByUserLoginId(delegator, userLoginId);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		String jsonOut = JSONUtil.bean2Json(posObj).toString();
		
		Map<String, Object> result = FastMap.newInstance();
		result.put("jsonOut", jsonOut);
        return result;
    }
     ...
     * @return
     */
    /*public static Map<String, Object> checkUserLogin(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Locale locale = (Locale)context.get("context");
		String jsonStr = (String)context.get("jsonStr");
		String userLoginId = "";
		String userLoginPwd = "";
        ...
        if (UtilValidate.isNotEmpty(jsonStr)) {
        	JSONObject jsonObj = JSONObject.fromObject(jsonStr);
        	userLoginId = (String) jsonObj.getString("userLoginId");
        	userLoginPwd = (String) jsonObj.getString("userLoginPwd");
        }
        
        BasePosObject posObj = null;
        try {
        	posObj = PartyWorker.checkUserLogin(dctx, locale, userLoginId, userLoginPwd);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    ...
    public static String toJSONStr(List<GenericValue> lists){
		String jsonOut = "";
		
		if (UtilValidate.isNotEmpty(lists)) {
			try {
				jsonOut = JSONUtil.listToJson(lists);
				JSON jsonObject = JSONSerializer.toJSON(jsonOut); 
    ...
    public static Map<String, Object> checkUserLogin(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        BasePosObject posObj = null;
        String localProductStoreId = "";
        try {
        	//初始化门店时local没有用户数据，需要访问总店服务器获得
        	String strServerType = System.getProperty("posstore.type");
        	if (strServerType.equals("1")) {	//本机为门店服务器
        		LocalDispatcher dispatcher = dctx.getDispatcher();
        		
	        	BasePosObject posStoreObj = SearchWorker.getStoreInfo(dctx.getDelegator());
	        	if (UtilValidate.isEmpty(posStoreObj)){	//初始化门店数据
	                Map<String,String> mInParam = FastMap.newInstance();

	                //生成调用web服务的参数
	                mInParam.put("jsonStr",jsonStr);

	                String strServerUrl = System.getProperty("posstore.server.url");
	                //调用总店web服务
	                String strJson = CommServiceUtil.sendRectWebService(strServerUrl+"/webtools/control/SOAPService/",
	                		"checkUserLogin", mInParam);
	                try {
	                	
	                	if (UtilValidate.isNotEmpty(JSONUtil.json2Jsonbj(strJson).get("data"))) {
		                	JSONObject jsonObject=(JSONObject)JSONUtil.json2Jsonbj(strJson).get("data");
		                	localProductStoreId = jsonObject.getString("productStoreId");
		                	String strStoreName = jsonObject.getString("storeName");
			                //根据总店取得数据保存门店store,同时从总店导入该门店用户数据
		                	SearchWorker.initStoreInfo(dctx,localProductStoreId,strStoreName);
			                SyncJob.importInitData2Store(dctx);
	                	}
	                } catch (Exception e){}
	        	} else {
	        		localProductStoreId = (String)((GenericValue)posStoreObj.getData()).get("storeId");
	        	}
        	}
        	if (UtilValidate.isEmpty(posObj)) {
        		posObj = PartyWorker.checkUserLogin(dctx, locale, userLoginId, userLoginPwd,localProductStoreId);
        		
        		if (UtilValidate.isNotEmpty(posObj) && UtilValidate.isNotEmpty(posObj.getData())) {
        			if (strServerType.equals("1")&&!((String)((GenericValue)posObj.getData()).get("productStoreId")).equals(localProductStoreId)){
            	        posObj.setFlag("F");
            	 		posObj.setMsg("not local store login");
        			}else{
        				String jsonOut = JSONUtil.bean2Json(posObj).toString();
        		...
        		if (!LoginServices.checkPassHis(dctx,userLoginId)){
    				posObj = new BasePosObject();
    				posObj.setFlag("U");
    				posObj.setMsg("check no data");
    				String jsonOut = JSONUtil.bean2Json(posObj).toString();
        	...
        	// 验证通过，如果已下载新版，进行更新安装，通知客户端
        	if (UtilValidate.isNotEmpty(posObj) && UtilValidate.isNotEmpty(posObj.getData())) {
        		if (!posObj.getFlag().equals("F")) {
	        		LocalDispatcher dispatcher = dctx.getDispatcher();
	        		Delegator delegator = dctx.getDelegator();
	    	        ...
	    	        if (!UtilValidate.isEmpty(netVersions)) {
	    	        	...
	    	        	if (newVersionF==1){
	            	        posObj.setFlag("I");
	            	 		posObj.setMsg("upgrading,restart later");
	            	 		String jsonOut = JSONUtil.bean2Json(posObj).toString();
	            			
	            			Map<String, Object> result = FastMap.newInstance();
	            			result.put("jsonOut", jsonOut);
	            			dispatcher.runAsync("upgradeNewVersion", null);
	            	        return result;
    ...
    public static Map<String,Object> updateUserPassword(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (UtilValidate.isNotEmpty(jsonStr)) {
        	...
        	currentPassword = (String) jsonObj.getString("userLoginPwd");
        	newPassword = (String) jsonObj.getString("userLoginNewPwd");
        }

        Map<String, Object> inMap = UtilMisc.<String, Object>toMap("userLoginId", userLoginId,
        		"currentPassword", currentPassword, "newPassword", newPassword, "newPasswordVerify", newPassword,
        		"locale", locale);
        Map<String, Object> resultPasswordChange = null;
        try {
            resultPasswordChange = dispatcher.runSync("updatePosPassword", inMap);
        } catch (Exception e) {
            Debug.logError(e, "Error calling updatePassword service", module);
            return ServiceUtil.returnError(e.getMessage());
        }
        if (ServiceUtil.isError(resultPasswordChange)) {
        	List<String> errorList = UtilGenerics.checkList(resultPasswordChange.get(ModelService.ERROR_MESSAGE_LIST));
        	String errorMessage = "error";
        	if (UtilValidate.isNotEmpty(errorList) && errorList.size()>0) {
    ...
    /**
     * 批量新增产品店铺角色：保存批量查询门店条件
     * @param dctx
     * @param context
     * @author ft
     * @return
     */
    public static Map<String, Object> addProductStoreRoleCon(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
		...
		Map<String, String> newRuleMap = UtilMisc.toMap("partyId", partyId);
		//rowCount为0表示无条件，delete表示删除条件
		if(rowCount.indexOf("0") > -1 || rowCount.indexOf("delete") > -1){
			try{
				//删除表中已有数据
				dels = delegator.findByAnd("ProductStorePartyRule", newRuleMap);
		...
		if(UtilValidate.isNotEmpty(fieldName)){
			...
			//获取序列
			seqnum = delegator.getNextSeqId("ProductStorePartyRule");
			
			gv = delegator.makeValue("ProductStorePartyRule", newRuleMap);
			...
			//根据查询条件拼出sql语句，存入ProductStorePartyRule
    		if ("storeName".equals(context.get("fieldName"))) {
    ...
    public static Map<String, Object> findStoreList(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String sqlString = "";
    	if (UtilValidate.isNotEmpty(partyId)) {
    		sqlString = condSqlString.getSqlString(delegator, "ProductStorePartyRule", "partyId", partyId);
    	}
...
//			listIt.close();
     ...
     * @return
     */
    public static Map<String, Object> createBatchProductStoreRoles(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
    	
    	String partyId = (String)context.get("partyId");
    	String roleTypeId = (String)context.get("batchRoleTypeId");
    	Timestamp fromDate = (Timestamp)context.get("batchFromDate");
    	
    	try{
    		//查询门店列表
    		Map<String, Object> storeMap = findStoreList(dctx,context);
    		EntityListIterator listIt = (EntityListIterator)storeMap.get("listIt");
    		List<GenericValue> lists = listIt.getCompleteList();
    		
    		for(GenericValue gv : lists){
    			//判断ProductStoreRole中是否已有店铺角色
    			List<GenericValue> proStoreRole = delegator.findByAnd("ProductStoreRole", UtilMisc.toMap("partyId",partyId,"roleTypeId",roleTypeId,"productStoreId",gv.get("productStoreId")));
    			
    			if(proStoreRole.size() == 0){
    				Map<String,Object> roleMap = FastMap.newInstance();
    				roleMap.put("partyId", partyId);
    				roleMap.put("roleTypeId", roleTypeId);
    				roleMap.put("productStoreId", gv.get("productStoreId"));
    				roleMap.put("fromDate", fromDate == null ? UtilDateTime.nowTimestamp():fromDate);
    				roleMap.put("thruDate", context.get("batchThruDate"));
    				roleMap.put("sequenceNum", context.get("sequenceNum"));
    				//保存店铺角色
    				GenericValue role = delegator.makeValidValue("ProductStoreRole", roleMap);
    				role.create();
    			}
     ...
     * @return
     */
    public static Map<String, Object> deleteBatchProductStoreRoles(DispatchContext dctx, Map<String, ? extends Object> context) {
    	Delegator delegator = dctx.getDelegator();
    	
    	String partyId = (String)context.get("partyId");
    	String roleTypeId = (String)context.get("roleTypeId");
    	
    	List<GenericValue> enitys = null;
    	try {
    		enitys = delegator.findByAnd("ProductStoreRole", UtilMisc.toMap("partyId", partyId, "roleTypeId", roleTypeId));
    		delegator.removeAll(enitys);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
