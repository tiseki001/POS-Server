...
public class GenericValue extends GenericEntity implements Reusable {
    ...
    public static GenericValue create(Delegator delegator, ModelEntity modelEntity, Map<String, ? extends Object> fields) {
    	Set<String> feildNames = fields.keySet();
    	Map<String, Object> fieldsNew = FastMap.newInstance();
    	for (String key : feildNames) {
			if(UtilValidate.isNotEmpty(fields.get(key))&&!"".equals(fields.get(key).toString()))
				fieldsNew.put(key, modelEntity.convertFieldValue(key,
							fields.get(key), delegator));
		}
    	GenericValue newValue = genericValueFactory.object();
        newValue.init(delegator, modelEntity, fieldsNew);
        return newValue;
    }
