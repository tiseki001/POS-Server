...
public abstract class EntityCondition extends EntityConditionBase implements IsEmpty, Reusable {
    ...
    public static EntityWhereString makeConditionWhere(String sqlString) {
    	try {
			...
			String helperBaseName = GenericDelegator.getGenericDelegator("default").getEntityHelper("party").getHelperName();
			DatasourceInfo datasourceInfo = EntityConfigUtil.getDatasourceInfo(helperBaseName);
			if(!datasourceInfo.fieldTypeName.equals("oracle")){
				//sqlString = "create_date>= to_date('2121212','YYYY-MM-DD') and aaa > 332 and drcreate < to_date('12123_eer','YYYY-mm-dd : frd') and a =b";
				int index=0;
				int index2=0;
				int index3=0;
				String str1;
				String str2;
				String str3;
				while(sqlString.toUpperCase().indexOf("TO_DATE(")>-1){
					index = sqlString.toUpperCase().indexOf("TO_DATE(");
					index2 = sqlString.indexOf(",", index);
					index3 = sqlString.indexOf(")", index2);
					str1 = sqlString.substring(0,index);
					str2 = sqlString.substring(index+8,index2);
					str3 = sqlString.substring(index3+1);
					sqlString = str1+str2+str3;
				}
			}
    	...
    	//delegator.getEntityHelper("product").
