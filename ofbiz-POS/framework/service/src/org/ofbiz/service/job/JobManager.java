...
public class JobManager {
    ...
    public synchronized List<Job> poll() {
    	...
    	//总店服务器不执行门店定义的上传下载及新版本下载任务
    	if (strServerType.equals("0")){
    		expressions.add(EntityCondition.makeCondition("serviceName", EntityOperator.NOT_IN, JobUtil.excludeIdList));
    	}
        ...
        if (UtilValidate.isNotEmpty(crashed)) {
            try {
                ...
                for (GenericValue job: crashed) {
                    ...
                    //对于还在running的job,对于sync的，更新状态，重新加载；其他类型job状态设置成crashed,不再重新加载
            	...
            	/*Iterator<GenericValue> iter = crashed.iterator();  
            	while(iter.hasNext()){  
            		GenericValue job = iter.next();  
                    if (JobUtil.checkSyncJob(job.getString("serviceName"))){
                    	job.remove();
                    }
            	}*/
