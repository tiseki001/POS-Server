...
public class PersistedServiceJob extends GenericServiceJob {
    ...
    private void createRecurrence(GenericValue job, long next) throws GenericEntityException {
        ...
        if (next > runtime) {
        	if (JobUtil.checkAddJob(job.getString("serviceName"), job.getString("jobName"), delegator)
        			&& !JobUtil.checkEbsJobByName(job.getString("jobName"))) {
        		...
        		job.set("runTime", new java.sql.Timestamp(next));
        		job.store();
        	}
        }
    ...
    protected void finish() throws InvalidJobException {
        ...
        try {
        	if (JobUtil.checkSyncJob(job.getString("serviceName"))
        			|| JobUtil.checkEbsJobByName(job.getString("jobName"))) {
                if (status == null || "SERVICE_RUNNING".equals(status) || "SERVICE_FINISHED".equals(status)
                		|| "SERVICE_CRASHED".equals(status)) {
                    job.set("statusId", "SERVICE_PENDING");
                    job.set("finishDateTime", null);
            		job.set("startDateTime", null);
            		job.set("finishDateTime", null);
    ...
    protected Map<String, Object> getContext() throws InvalidJobException {
        ...
        try {
            ...
            if (!UtilValidate.isEmpty(jobObj.getString("runtimeDataId"))) {
                ...
                if (contextObj != null) {
                	//门店下载上传及自动更新任务参数非xml格式
                	String serviceName = jobObj.getString("serviceName");
                	if (!UtilValidate.isEmpty(serviceName) && (serviceName.equals("import2StoreJob")
                			|| serviceName.equals("uploadData2Store")
                			|| serviceName.equals("downloadNewVersion"))){
                		context = FastMap.newInstance();
                		String value = contextObj.getString("runtimeInfo");
                		if (UtilValidate.isNotEmpty(value))
                			context.put("entityKey",contextObj.getString("runtimeInfo"));
                	} else {
